...
module JobStats
  ...
  ActiveSupport::Notifications.subscribe(/enqueue(_at)?\.active_job/) do |name, _started, _finished, _unique_id, _data|
    job_enqueued
    Rails.logger.info "JobStats: Enqueued job #{name}"
  end

  ActiveSupport::Notifications.subscribe 'demo.hash' do |_name, _started, _finished, _unique_id, data|
    Rails.logger.info "JobStats: Got new hash #{data}"
    with_stats(true) do |stats|
      count, str, hash = data
